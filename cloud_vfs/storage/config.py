...
from cloud_vfs.storage.legacy_env import remote_azure_env
...
def resolve_archive(
    ...
    if archive_name == "remote_staging":
        return ArchiveConfig(
            name=archive_name,
            provider="azure",
            bucket=remote_azure_env(env, "AZ_REMOTE_CONTAINER", block.get("container")) or "data",
            region=remote_azure_env(env, "AZ_REMOTE_LOC", block.get("region")),
            account=remote_azure_env(
                env, "AZ_REMOTE_STORAGE_ACCOUNT", block.get("account")
            ),
            key=remote_azure_env(env, "AZ_REMOTE_STORAGE_KEY"),
        )
    raise ValueError(f"Unknown archive: {archive_name}")

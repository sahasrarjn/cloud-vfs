#!/usr/bin/env bash
# Provision Azure storage accounts from .cloud-vfs/config.env
set -euo pipefail
...
echo "==> Remote staging: ${AZ_REMOTE_STORAGE_ACCOUNT} (${AZ_REMOTE_LOC})"
REMOTE_KEY=$(provision_storage_account "$AZ_REMOTE_RG" "$AZ_REMOTE_LOC" \
  "$AZ_REMOTE_STORAGE_ACCOUNT" "$AZ_REMOTE_CONTAINER")

SECRETS="$(secrets_file)"
cat > "$SECRETS" <<EOF
# Auto-generated — do not commit
AZ_LOCAL_STORAGE_KEY='$LOCAL_KEY'
AZ_REMOTE_STORAGE_KEY='$REMOTE_KEY'
EOF
chmod 600 "$SECRETS"

echo
echo "Local archive:  $(az_local_blob_url)"
echo "Remote staging: $(az_remote_blob_url)"
